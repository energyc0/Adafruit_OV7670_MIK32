...
#include "arch/mik32/i2c_bitbang.h"
#include "arch/mik32/mik32.h"
#include "arch/mik32/sccb.h"
#include "arch/mik32/uart.h"
#include "mik32_hal.h"
...
static Pin scl = {.gpio = GPIO_1, .pin_num = 13};
static Pin sda = {.gpio = GPIO_1, .pin_num = 12};
static Pin d0 = {.gpio = GPIO_0, .pin_num = 0};
static Pin d1 = {.gpio = GPIO_0, .pin_num = 1};
static Pin d2 = {.gpio = GPIO_0, .pin_num = 2};
static Pin d3 = {.gpio = GPIO_0, .pin_num = 3};
static Pin d4 = {.gpio = GPIO_0, .pin_num = 4};
static Pin d5 = {.gpio = GPIO_0, .pin_num = 5};
static Pin d6 = {.gpio = GPIO_0, .pin_num = 9};
static Pin d7 = {.gpio = GPIO_0, .pin_num = 10};
static Pin xclk = {.gpio = XCLK_PIN_GPIO, .pin_num = XCLK_PIN_NUM};
static Pin pclk = {.gpio = PCLK_PIN_GPIO, .pin_num = PCLK_PIN_NUM};
static Pin vsync = {.gpio = GPIO_1, .pin_num = 0};
static Pin href = {.gpio = GPIO_1, .pin_num = 2};

static void SystemClock_Config();
static void GPIO_Init();
static void Pins_Init(OV7670_pins* pins);
static int Try_Read_Registers(); // Try read registers of camera, return 1 if
...
int main() {
  ...
  if (OV7670_begin(&host, CAMERA_COLORSPACE, CAMERA_DIV_FACTOR, 1.0) !=
      OV7670_STATUS_OK) {
  ...
  while (1) {
    OV7670_capture((uint32_t*)framebuffer, CAMERA_WIDTH, CAMERA_HEIGHT,
                   &vsync.gpio->STATE, 1 << vsync.pin_num, &href.gpio->STATE,
                   1 << href.pin_num);
