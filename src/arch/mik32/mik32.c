...
__attribute__((section(".ram_text"), always_inline)) static inline uint8_t
ov7670_read_byte() {
  /* Wait for clock */
  ...
  // register uint8_t byte = (gpio_state & 0x3F) | (((gpio_state & 600) >> 3) &
  // 0xC0);
...
__attribute__((section(".ram_text"), always_inline)) static inline uint16_t
ov7670_read_pixel() {
...
__attribute__((section(".ram_text"))) void OV7670_capture(
    uint32_t* dest, uint16_t width, uint16_t height,
