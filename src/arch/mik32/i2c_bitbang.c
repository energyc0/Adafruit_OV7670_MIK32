...
#define SDA_OUTPUT()                       \
  do {                                     \
    SDA_GPIO->DIRECTION_OUT = SDA_PIN_BIT; \
  } while (0)
#define SDA_INPUT()                       \
  do {                                    \
    SDA_GPIO->DIRECTION_IN = SDA_PIN_BIT; \
  } while (0)
#define SDA_HIGH()               \
  do {                           \
    SDA_GPIO->SET = SDA_PIN_BIT; \
  } while (0)
#define SDA_LOW()                  \
  do {                             \
    SDA_GPIO->CLEAR = SDA_PIN_BIT; \
  } while (0)
#define SDA_READ() (SDA_GPIO->STATE & SDA_PIN_BIT)

#define SCL_OUTPUT()                       \
  do {                                     \
    SCL_GPIO->DIRECTION_OUT = SCL_PIN_BIT; \
  } while (0)
#define SCL_HIGH()               \
  do {                           \
    SCL_GPIO->SET = SCL_PIN_BIT; \
  } while (0)
#define SCL_LOW()                  \
  do {                             \
    SCL_GPIO->CLEAR = SCL_PIN_BIT; \
  } while (0)
#define SCL_READ() (SCL_GPIO->STATE & SCL_PIN_BIT)

__attribute__((section(".ram_text"))) static inline void I2C_BitBang_WriteBit(
    uint8_t bit) {
  if (bit)
    SDA_HIGH();
  else
...
__attribute__((section(".ram_text"))) static inline uint8_t
I2C_BitBang_ReadBit() {
  uint8_t bit;
...
__attribute__((section(".ram_text"))) uint8_t
I2C_BitBang_WriteByte(uint8_t byte, uint8_t is_start, uint8_t is_stop) {
  uint8_t ack, i;
  if (is_start)
    I2C_BitBang_Start();
...
__attribute__((section(".ram_text"))) uint8_t
I2C_BitBang_ReadByte(uint8_t ack_bit, uint8_t is_stop) {
  uint8_t byte = 0;
